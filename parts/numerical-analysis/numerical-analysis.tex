...
\newpar
However, evaluating the interpolation polynomial can be difficult and unstable in practice. We now focus our attention on deriving a scheme that allows a stable evaluation of $p(x)$ in $\cO(n^2)$ time.
...
\end{definition}
\begin{exercise}
	A quadrature formula is exact of degree $0$ if and only if $\sum_{i = 0}^n w_i = b-a$.
\end{exercise}
\begin{lemma}
...
\begin{theorem}
	...
	\begin{align*}
		T(f) = \frac{b-a}{6}\lr(f(a) + 4f\lr(\frac{a+b}{2})+ f(b)).
	\end{align*}
...
\end{lemma}
\begin{proof}
	Let $Q(f) = \sum_{i = 0}^n w_i \cdot f(x_i)$ be our quadrature formula. Let $p(x) = \prod_{i = 0}^n (x - x_i)^2$. Then we have $Q(p) = 0$, but $I(p) > 0$. Since $p \in \cP_{2n + 2}$, $Q$ cannot have exactness $2n + 2$.
\end{proof}
\begin{lemma}
...
\begin{proof}
	\begin{enumerate}
		\item If $Q$ is exact of degree $n$, it must integrate the lagrange polynomials exactly. We get:
		\begin{align*}
			\int_a^b L_i(x) ~dx 
			&= \sum_{j = 0}^n w_j L_i(x_j)\\ 
			&= \sum_{j = 0}^n w_j \delta_{ij}\\ 
			&= w_i\\ 
		\end{align*}
		\item Any quadrature formula $Q$ that integrates the Lagrange polynomials exactly must also integrate any linear combination of Lagrange polynomials exactly. Since the Lagrange polynomials at $n+1$ nodes form a basis of $\cP_n$, it follows that $Q$ integrates any polynomial of degree $n$ exactly.
	\end{enumerate}
\end{proof}
\begin{lemma}
	If a quadrature formula is exact of even degree $n = 2m$, we have $w_i > 0$ for all $i$.
\end{lemma}
\begin{proofsketch}
	\begin{align*}
		w_i = \int_a^b L_i(x)^2 > 0
	\end{align*}
\end{proofsketch}

We will now devote our attention to deriving the \tib{Gauß quadrature}, which chooses $n+1$ quadrature points such that the maximum possible exactness degree is achieved.
\begin{proposition}
	Let $\omega \in C^0([a,b])$ with $\omega(x) > 0$. Then $\scalar{f}{g}_\omega = \int_a^b f(x) g(x) \omega(x) ~dx$ is a scalar product on $C^0([a,b])$.
\end{proposition}
Using Gram-Schmidt starting with the standard monomial basis, we can construct an $\omega$-orthonormal base $(\pi_i)_{i \in [0..n]}$ of $\cP_n$ such that $\pi_i \in \cP_i$ for all $i$.
\begin{proposition}
	Using $\omega(x) = (1-x^2)^{-\frac{1}{2}}$ on $(-1,1)$ gives the Chebyshev polynomials.
\end{proposition} 
\begin{proposition}
	Using $\omega(x) = 1$ on $[-1,1]$ gives the Legendre polynomials.
\end{proposition}
\begin{lemma}
	Every orthonormal polynomial $\pi_i$ has $i$ roots of order $1$ lying in the given interval $(a,b)$.
\end{lemma}
\begin{importanttheorem}{}{}
	Let $x_i$ be the roots of $\pi_{n+1}$ for some $\omega \in C^0([a,b])$. Then the quadrature formula with roots $x_i$ and weights
	\begin{align*}
...
\end{theorem}
\begin{proposition}
	The bisection algorithm converges linearly.
\end{proposition}
\begin{theorem}
...
\end{theorem}
\begin{proposition}
	The secant method converges superlinearly, but subquadratically, with convergence order $\phi = \frac{1 + \sqrt{5}}{2}$ (the golden ratio!)
\end{proposition}
\noindent
