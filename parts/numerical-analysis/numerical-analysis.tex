...
A \tib{summed quadrature formula} is a function $T(h) : \bR_{\geq 0} \to \bR$ which gives the integral of a function $f \in C^k([a,b])$ in the limit $h \to 0$. If the error of our quadrature formula is of order $h^\gamma$ for a $\gamma \in \bN$, we have
\begin{align*}
	T(h) = T(0) + \phi(h)
\end{align*}
