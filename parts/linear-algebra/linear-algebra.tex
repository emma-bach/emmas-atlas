...
\begin{importantdefinition}{Vector Spaces}{}
	...
	\end{enumerate}
\end{importantdefinition}
Spelling everything out, the condition that $(V,+)$ is a commutative group tells us that:
\begin{enumerate}
...
\end{enumerate}
The second condition might seem a bit more obtuse - however, all it tells us is that $.$ assigns to each scalar $\lambda \in \bF$ an endomorphism of $V$, such that $1_\bF.$ is the identity map $\id_\bF$. Putting everything together, we get the following demands for scalar addition:
\begin{enumerate}
	\item Scalar multiplication by any scalar respects vector addition, i.e. the operations distribute:
	\begin{align*}
		\forall &\lambda \in \bF:\\
		\forall &v,w \in V:\\
