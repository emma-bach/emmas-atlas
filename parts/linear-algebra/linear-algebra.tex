...
\begin{importanttheorem}{Intercept Theorem}{}
	Let $v_1, v_2 \in \bR$ be linearly independent. Let $L_1$ be a line through $v_2$ and $v_1$. Then if we have $\alpha, \beta \in \bR_{\neq 0}$, and $L_2$ is a parallel line through $\alpha v_2$ and $\beta v_1$, we must have $\alpha = \beta$.
\end{importanttheorem}
\begin{proof}
...
\end{proof}
\begin{theorem}
	Every injective map $\Phi : \bR^2 \to \bR^2$ mapping $0$ to $0$ and affine lines to affine lines must be linear.
\end{theorem}
\begin{proof}
	Since $\Phi$ is linear with $\Phi(0) = 0$, it must map lines through the origin to lines through the origin. Let
	\begin{align*}
		\Phi(e_1) &:= v_1\\
	...
	our map $\phi : \bR \to \bR$ must be a field homomorphism, of which the identity is the only one.
\end{proof}
...
\begin{definition}
	Let $E$ be a Euclidean plane. Then the \tib{extended Euclidean plane} is the set
	\begin{align*}
