...
Intuitively, a vector space is any algebraic structure $V$ equipped with a field $\bF$, such that elements of $V$ can be added to elements of $V$ or multiplied by elements of $\bF$, and these operations behave how vector addition and scalar addition behave in $\bR^n$. More precisely:
\begin{importantdefinition}{Vector Spaces}{}
	A \tib{vector space $V$ over a field of scalars $\bF$} is a structure $(V,\bF,+,.)$ consisting of a \tib{vector addition operation} $+ : V \times V \to V$ and a \tib{scalar multiplication operation} $. : \bF \times V \to V$ such that:
	\begin{enumerate}
		\item vectors form a commutative group under vector addition, and
		\item scalar multiplication is a homomorphism from $\bF$ into the endomorphism ring of $(V,+)$.
	\end{enumerate}
\end{importantdefinition}
The condition that $(V,+)$ is a commutative group should be simple to understand by now - it tells us that:
\begin{enumerate}
	...
	\begin{align*}
		...
		&v + w = w + v
	\end{align*}
	\item There exists a vector $0_V \in V$ such that $v + 0_V = v$ for all $v \in V$.
	\item For every vector $v \in V$, there exists an additive inverse $-v \in V$:
	\begin{align*}
		\forall &v \in V:\\
		\exists &-v \in V:\\
		&v + (-v) = 0_V
	\end{align*} 
	As usual, we write $v - w := v + (-w)$ as an abbreviation.
\end{enumerate}
The second condition might seem a bit more obtuse - however, all it tells us is that $.$ assigns to each scalar $\lambda \in \bF$ an endomorphism $\lambda. : V \to V$, such that:
\begin{enumerate}
	\item Each multiplication operation is an endomorphism, i.e. scalar multiplication distributes over vector addition:
	\begin{align*}
		\forall &\lambda \in \bF:\\
		\forall &v,w \in V:\\
		&\lambda.(v+w) = \lambda.v + \lambda.w
	\end{align*}
	\item The map assigning operators to scalars is a homomorphism, i.e. scalar multiplication also distributes over scalar addition:
	\begin{align*}
		\forall &\lambda, \mu \in \bF:\\
		\forall &v \in V:\\
		&(\lambda + \mu).v = \lambda.v + \mu.v
	\end{align*}
	\item Scalar multiplication by the field identity $1_\bF$ leaves vectors unchanged:
	\begin{align*}
		\forall &v \in V:\\
		&1_\bF.v = v
	\end{align*}
...
\begin{importanttheorem}{Characterstic Polynomial}{}
	...
	\begin{align*}
		\chi_F(\lambda) = \det(\lambda.\id_V - F) = 0_\Bbbk
	\end{align*}
...
\end{exercise}
\section{Algebras, Cayley-Hamilton and Frobenius}
\begin{importantdefinition}{Algebra over a Ring}{}
	An \tib{algebra over a commutative ring $R$} is an $R$-module $A$ equipped with a bilinear multiplication operation $\cdot : A \times A \to A$. We call $A$ commutative, associative or unital if the multiplication operator $\cdot$ is commutative, associative, or has an identity element.
\end{importantdefinition}
\begin{exercise}
	\phantom{}
	\label{ex:realfunctionalgebra}
	\begin{enumerate}
		\item Show that every commutative ring trivially forms an algebra over itself.
		\item Show that the set $C(M, \bR)$ of continuous functions $M \to \bR$, where $M$ is a metric space, forms a commutative associative unital algebra under pointwise addition and multiplication.
	\end{enumerate}
\end{exercise}
\begin{importanttheorem}{Polynomial Evaluation Map}{}
	Let $R$ be a commutative ring, and let $A$ be a commutative unital algebra over $R$. Then, for every $a \in A$, there exists a unique algebra homomorphism $\ev_a : R[X] \to A$ such that:
	\begin{enumerate}
...
\begin{importantcorollary}{Generalized Eigenspace Decomposition}{geneigenspace}
...
\end{corollary}
\begin{proof}
	Generalized Eigenspace decomposition \ref{th:geneigenspace} shows us that $V$ is the direct sum of the generalized Eigenspaces of $F$.
...
\iffalse
\chapter{A more bottom-up approach*}
This chapter will focus on exploring why our various constructions (scalar products, orthogonal endomorphisms, etc.) are the "correct" objects to match our geometric intuition. This chapter follows the script "Lineare Algebra II", by Prof. Dr. Wolfgang Soergel.
\begin{definition}
	Let $V$ be a real vector space. Then a \tib{ray} is a subset of the form
	\begin{align*}
		[v\rangle := \lr{\lambda.v \mid \lambda \in \bR_{\geq} 0}.
	\end{align*}
\end{definition}
\begin{importantdefinition}{Linear Congruence Group}{}
	Let $E$ be a two-dimensional real vector space. Then we call a subgroup $C \subseteq \GL(E)$ a \tib{linear congruence group on $Z$} if for any pair of rays $A,B \subseteq E$, there are exactly two elements $k,h \in C$ mapping $A$ to $B$.
\end{importantdefinition}
\begin{importanttheorem}{Orthogonal maps are linear congruences}{}
	Given a two-dimensional real vector space with scalar product $(E,s)$, the orthogonal group $O(E,s)$ is a linear congruence group.
\end{importanttheorem}
\begin{proof}
	Given two rays $A,B \subseteq E$, choose normalized vectors $v_a \in A, v_b \in B$. Now, we can add vectors $w_a, w_b$ that are orthogonal to $v_a$ and $v_b$ to form bases of $E$. Since orthogonal maps preserve lengths, an orthogonal map taking $A$ to $B$ must map $v_a$ to $v_b$, and since they also preserve orthogonality, the only remaining options are to map $w_a$ to either $w_b$ or $-w_b$.
\end{proof}
The rest of this chapter will be devoted to showing that $O(E,s)$ is the \tit{only} linear congruence group on $E$.
\begin{definition}
	Let $E$ be a two-dimensional real vector space.
	We call an element $r \in \GL(E)$ a \tib{reflection} if its set of fixed points is a line and its square is the identity.
\end{definition}
\begin{proposition}
	Given a two-dimensional real vector space with linear congruence group $(E,C)$, $C$ contains all reflections of $E$.
\end{proposition}
\begin{lemma}
	\label{lemma:orientationreflection}
	The orientation-reversing elements of a linear congruence group are exactly the reflections.
\end{lemma}
For the following lemmas, let $(E,C)$ be a two-dimensional real vector space with congruence group.
\begin{lemma}
	For any two rays $A,B \subseteq E$, there exists exactly one reflection in $C$ mapping each one to the other.
\end{lemma}
\begin{proof}
	Let $c$ be a congruence mapping $A$ to $B$. Let $r_B$ the the reflection at $B$. Then clearly $r_Bc \neq c$, so $r_Bc$ and $c$ must be the two congruences mapping $A$ to $B$. Since $r_B$ has determinant $-1$, either $r_Bc$ or $c$ must be orientation-reversing, and must thus be a reflection by \ref{lemma:orientationreflection}. Since the square of a reflection is the identity, $r$ is self-inverse, and must thus not only map $A$ to $B$, but also $B$ to $A$.
\end{proof}
\begin{lemma}
	Every element of $C$ is either a reflection or the composition of two reflections in $C$.
\end{lemma}
\begin{proof}
	As before, either $r_Bc$ or $c = r_B(r_Bc)$ must be a reflection.
\end{proof}
\begin{lemma}
	Every element of $C$ has determinant $\pm 1$.
\end{lemma}
\begin{proof}
	Since reflections are self-inverse, they must have $\pm 1$ as their eigenvalues, and thus have determinant $\pm 1$. Since they generate $C$, the same holds for the entire group.
\end{proof}
\begin{lemma}
	The elements of $C$ with determinant $1$ form a subgroup $C^+$.
\end{lemma}
\begin{lemma}
	For every reflection $r \in C$ and every $c \in C^+$, we have $rcr = c^{-1}$.
\end{lemma}
\begin{lemma}
	$C^+$ is abelian.
\end{lemma}
...
\end{lemma}
\begin{lemma}
	There are exactly two elements $r \in C$ such that $r^2 = -\id_E$.
\end{lemma}
\fi
...
\begin{definition}
	Let $V$ be a vector space. Then a \tib{linear hyperplane} is a vector subspace of $W$ of codimension $1$, i.e.
	\begin{align*}
		\codim(W \subseteq V) := \dim(V / W) = 1
	\end{align*}
\end{definition}
Equivalently, $W \neq V$ such that $V$ is generated by $W$ and any vector $v \in V \setminus V$. If $V$ is finite-dimensional, we have $\dim(V/W) = \dim V - \dim W$, and thus a linear hyperplane is a subspace $W$ whose dimension is one lower than the dimension of $V$. An \tib{affine Hyperplane} is defined entirely analogously as an affine subspace of codimension one.
\begin{exercise}
	\begin{enumerate}
		\item In any affine space, the intersection of an affine line with another affine subspace is either empty, a single point, or the entire line.
		\item For any affine hyperplane $H$ and any line $g$ with directional vector $\vv \notin \vH$, the intersection of $g$ and $H$ consists of a single point.
	\end{enumerate}
\end{exercise}
\begin{definition}
	Given an affine space $E$, an affine hyperplane $H \subset E$, and a vector $\vv \in \vE \setminus \vH$, the \tib{parallel projection of $E$ onto $H$ in direction $\vv$} is the map 
	\begin{align*}
		\pi = \pi_{\vv} = \pi_{\vv,H} : E \to H
	\end{align*}
	assigning to each point $e$ the intersection of the affine line $(e + \angles{\vv})$ with $H$.
\end{definition}
\begin{definition}
	Given an affine space $E$, an affine hyperplane $H \subset E$, and a point $q \in E \setminus H$, the \tib{central projection of $E$ onto $H$ centered at $q$} is the map 
	\begin{align*}
		\zeta = \zeta_q = \zeta_{q,H} : E \setminus(q + \vH) \to H
	\end{align*}
	assigning to each point $e \in E$ the intersection of the line though $e$ and $q$ with $H$.
\end{definition}
\begin{exercise}
	The image of a line $g$ under central projection is either:
	\begin{enumerate}
		\item A line,
		\item a pointed line, i.e. a line with a single missing point (the \tib{vanishing point}),
		\item a single point,
		\item or the empty set.
	\end{enumerate}
\end{exercise}
\section{Projectivization}
\begin{importantdefinition}{Projectivization}{}
	Let $\Bbbk$ be a field. Let $V$ be a $\Bbbk$-vector space. Then the set of all lines through the origin in $V$ is called the the \tib{projectivization of $V$} and denoted by $\bP V$.
\end{importantdefinition}
In the finite-dimensional case, we have $\dim \bP V = \dim V - 1$. It is therefore common to write
\begin{align*}
	\bP^n \bk := \bP(\Bbbk^{n+1})
\end{align*} 
Beware, this means $\bP \bk \neq \bP^1 \bk$! The set $\bP^1 \bk$ is called the \tib{projective line over $\bk$}, the set $\bP^2 \bk$ is called the \tib{projective plane over $\bk$}.
\begin{definition}
	Given a vector space $V$, we call a subset $g \subseteq \bP V$ a \tib{projective line} or \tib{$\bP$-line} if there exists a vector subspace $g' \subseteq V$ such that $\dim g' = 2$ and $g = \cP g'$.
\end{definition}
\begin{lemma}
	For any vector space $V$, the set $\bP V$ forms an incidence geometry with $\bP$-lines as the lines.
\end{lemma}
\begin{lemma}
	In a projective plane over any field, any two different $\bP$-lines intersect in exactly one point.
\end{lemma}
\begin{definition}
	An \tib{embedding of incidence geometries} is an injective map $X \inj Y$ such that points in $X$ are colinear if and only if their images are colinear. A bijective embedding of incidence geometries is known as a \tib{colineation}.
\end{definition}
\begin{lemma}
	Any injective vector space homormophism $W \inj V$ induces an embedding $\bP W \inj \bP V$.
\end{lemma}
\begin{importantdefinition}{}{}
	...
	\begin{enumerate}
		\item Sets $ \bV g := g \sqcup \lr{\vg}$ for any affine line $g \in E$,
		\item and $\bP$-lines $l = \bP l'$.
	\end{enumerate}  
	 We call this structure the \tib{projective completion of $E$}. We call points in $\bP \vE$ \tib{points at infinity}.
\end{importantdefinition}
\begin{exercise}
	If $E$ is a two-dimensional affine space, then its projective completion $\bV E$ contains exactly one line containing only infinitely distant points. We call this line the \tib{line at infinity}.
\end{exercise}
\begin{exercise}
	Given an affine space $E$, the trivial embeddings $E \inj \bV E$ and $\bP \vE \inj \bV E$ are embeddings of incidence geometries. Any injective affine map $F \inj E$ of affine spaces is an embedding of incidence geometries and induces an embedding $\bV E \inj \bV F$ of their projective completions.
\end{exercise}
\begin{importanttheorem}{}{}
	...
	\end{align*}
\end{importanttheorem}
\begin{importantcorollary}{Projective lines are projective completions }{linecompletion}
	Let $F$ be a field. Then we have $\bP^1 F \cong \bV F = F \sqcup \lr{\infty}$.
\end{importantcorollary}
\begin{exercise}
	\begin{enumerate}
		\item For any field $\bF$, we have
		\begin{align*}
			\bP^n \bF \cong \bigsqcup_{i = 0}^n \bF^i
		\end{align*}
		\item For a finite Field $\bF_q$, we have
		\begin{align*}
			\abs{\bP^n \bF_q} =
			\sum_{i = 0}^n q^{i}
		\end{align*}
...
\begin{proposition}
	...
	\begin{enumerate}
		...
		\item Different elements of $K_H$ are still different when restricted to $H$.
	\end{enumerate}
\end{proposition}

\clearpage

\section{Complex Möbius Geometry}
\begin{definition}
	We denote by $\gamma$ the "double complex conjugation map"
	\begin{align*}
		\gamma : \bC^2 &\to \bC^2\\
		(w,z) &\mapsto (\ol w, \ol z)
	\end{align*}
\end{definition}
\begin{theorem}
	The set of all antilinear automorphisms of $\bC^2$ is given by
	\begin{align*}
		\GL\lr(\bC^2)\gamma := \lr{F \circ \gamma \mid F \in \GL\lr(\bC^2)}
	\end{align*}
\end{theorem}
\begin{definition}
	We denote by $$\GL\lr(\bC^2)\angles{\gamma} := \GL\lr(\bC^2) \sqcup \GL\lr(\bC^2)\gamma$$
	the set of all automorphisms of $\bC^2$ that are either linear or antilinear.
\end{definition}
Since $\bP^1 \bC$ consists of subsets of $\bC^2$, every map $\bC^2 \to \bC^2$ mapping lines to lines acts on $\bP^1 \bC$ in a trivial way. By theorem \ref{th:linecompletion} $\bP^1 \bC \cong \hat \bC$, the map 
\begin{align*}
	\iota_\bC : 
	\hat \bC &\iso \bP^1 \bC \inj \bC^2\\
	z &\mapsto (z,1)\\
	\infty &\mapsto (1,0)
\end{align*}
is a bijection, which means we also get an action on $\hat \bC$.
\begin{importanttheorem}{Action of $\GL\lr(\bC^2)\angles{\gamma}$ on $\hat \bC$}{}
	The canonical action gives us a group homomorphism
	\begin{align*}
		\phi : \GL(2; \bC)\angles{\gamma}&\to \Ens^\times(\hat \bC)\\
		\gamma &\mapsto \lr(z \mapsto \ol z)\\
		M :=
		\begin{pmatrix}
			a & b\\
			c & d
...
\begin{proof}
	\begin{enumerate}
		\item Let $z \in \bC$. Translating directly from $\bC$ to gives $\bC^2$ $\iota_\bC(z) = \binom{z}{1}$, which means $M(\iota_\bC(z)) = \binom{az + b}{cz + d}$. 
		\begin{enumerate}
			\item If $cz + d$, then this point lies on the same line through the origin as $\binom{\frac{az + b}{cz + d}}{1}$, which is then translated back from $\bP^1 \bC$ to $\hat \bC$ as the point $\frac{az + b}{cz + d}$.
			\item If $cz + d = 0$, then this point lies on the same line through the origin as $\binom{1}{0}$, which is then translated back from $\bP^1 \bC$ to $\hat \bC$ as the point $\infty$.
		\end{enumerate}
		\item If $z = \infty$, then $\iota_\bC(\infty) = \binom{1}{0}$, which gets mapped to $\binom{a}{c}$, from which we can proceed as before.
	\end{enumerate}
\end{proof}
\begin{theorem} 
	Our canonical group homomorphism $\phi : \GL\lr(\bC^2)\angles{y} \to \Ens^\times(\hat \bC)$ is a surjective group homomorphism
	\begin{align*}
		\phi : \GL\lr(\bC^2)\angles{\gamma} \surj \tn{Möb}(\hat \bC)
	\end{align*}
	whose kernel is the group consisting of all scalar multiplies of the identity.
\end{theorem}
\begin{proof}
	\begin{enumerate}
		\item
		Since $\GL\lr(\bC^2)$ is generated by the diagonal matrices together with the matrices $S = \begin{pmatrix}
		0 & 1\\
		1 & 0
		\end{pmatrix}$ 
		and $T = \begin{pmatrix}
		1 & 1\\ 0 & 1\end{pmatrix}$, $\GL(2,\bC)\angles{\gamma}$ is generated by these matrices along with $\gamma$. It therefore suffices to show that the images of these matrices are Möbius transformations. Now, according to our previous analysis of the maps generated by $\phi$, these images are as follows:
		\begin{enumerate}
			\item A diagonal matrix with entries $\lambda$ and $\mu$ gets mapped to $\frac{\lambda}{\mu} \cdot z$, which corresponds to a scaled rotation.
			\item $S$ gets mapped to $z \mapsto \frac{1}{z}$, which is the circle inversion at the unit circle composed with a reflection along the real axis.
			\item $T$ gets mapped to the translation $z \mapsto z + 1$.
			\item $\gamma$ gets mapped to the conjugation $z \mapsto \ol z$, which is just the reflection along the real axis.
		\end{enumerate}
		All of these maps are known Möbius transformations.
		\item We now know that the image of $\phi$ contains inversion at the unit circle, all translations, and all scalings. Therefore, it contains all inversions along proper circles. It also contains reflection along the extended real axes and all rotations at the origin, so it also contains reflections along extended lines. Thus´, the image contains the generators of $\tn{Möb}(\hat \bC)$, and therefore the entire group.
		\item 
		An antilinear automorphism cannot stabilize every one-dimensional subspace. Thus, antilinear automorphisms cannot be mapped the identity, i.e. they cannot be contained in the kernel.
	...
	\end{enumerate}
\end{proof}
\begin{importantdefinition}{Projective Linear Groups}{}
	Given a vector space $V$, the \tib{projective linear group} $\tn{PGL}(V)$ is defined as the quotient group of $\GL(V)$ by the group $Z(V)$ of all scalar multiples of the identity. The \tib{projective special linear group} $\tn{PSL}(V)$ is analogously defined as the quotient group of $\SL(V)$ by the group $SZ(V)$ of all multiples of the identity by a unit scalar.
\end{importantdefinition}
\begin{importanttheorem}{Möbius group as a projective linear group}{}
	\begin{align*}
		\tn{Möb}^+(\hat \bC) \cong \PGL\lr(\bC^2)
	\end{align*}
\end{importanttheorem}
\begin{definition}
	We denote by $\tau$ reflection along the  axis $\angles{e_1}$, i.e.
	\begin{align*}
		\tau : \bR^2 &\to \bR^2\\
		\binom{a}{b} &\mapsto \binom{a}{-b}
	\end{align*}
\end{definition}
\begin{importanttheorem}{Action of $\SL(\bC^2)$ on $\hat \bC$}{}
	Our surjective group homomorphism $\phi : \GL(\bC^2)\angles{\gamma} \surj \Moeb\lr(\bC^2)$ induces a surjective group homomorphism
	\begin{align*}
		\SL(\bR^2)\angles{\tau \gamma} \surj \Moeb_\bH(\hat \bC)
	\end{align*}
	whose kernel is $\pm \Id_{\bR^2}$.
\end{importanttheorem}
\begin{importanttheorem}{Conjugations of $\bH$ as a projective special linear group}{}
	\begin{align*}
		\tn{Möb}^+_\bH(\hat \bC) \cong \PSL\lr(\bR^2)
	\end{align*}
\end{importanttheorem}