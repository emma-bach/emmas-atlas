...
\begin{importantdefinition}{Angle between two vectors}{}
	Given two vectors $\vx,\vy \in \bR^n \setminus \lr{\vzero}$, we define the \tib{angle} between them to be:
	\begin{align*}
...
\fi
\newpar
We have to be content with this restricted interval $[0,\pi]$, since $\cos$ isn't injective on larger intervals, and thus $\arccos$ wouldn't be well-defined on the full interval $(-\pi,\pi]$.
\newpar 
This means that these inner product-based angles always measure the smaller of the two angles between the vectors, disallowing negative angles and angles larger than $\pi$.
\newpar
...
\section{How Quaternions represent the Geometry of $\bR^3$}
