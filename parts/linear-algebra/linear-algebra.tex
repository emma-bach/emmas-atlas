...
\end{theorem}
\begin{theorem}
	Let $(A, \vA, g)$ be a finite-dimensional normed $\Bbbk$-affine space over a vector space with scalar product. Let $F : A \to A$ be an affine sometry. Then there exists a point $b \in A$ and a vector $\vv \in \Eig(\vF, 1)$ such that
	\begin{align*}
		F(a) = b + \vv + \vF(\vec{ba})
	\end{align*}
	$\vv$ is uniquely determined by $A$, while $b$ is an arbitrary point chosen from an affine-$F$-invariant subspace $(B, \Eig(\vF, 1)) \subseteq (A, \vA)$. 
\end{theorem}
\begin{definition}
	We call $(B, \Eig(\vF, 1))$ the \tib{axis set} of $F$, and call lines of the form
	\begin{align*}
		\lr{b + \lambda.\vv \mid \lambda \in \Bbbk} \subseteq B
	\end{align*}
	\tib{axes} of $F$.
\end{definition}
