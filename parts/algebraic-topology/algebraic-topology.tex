...
\begin{importantdefinition}{}{}
	Let $X,Y$ be topological spaces. Let $f,g : X \to Y$ be continuous. Then $f$ is called \tib{homotopic} to $g$ via a map $h$ iff. $h$ is a "continuous interpolation between $f$ and $g$", i.e:
	\begin{enumerate}
...
\end{importantdefinition}
$\pi^{-1}[x]$ is called the \tib{Fiber of $\pi$ at $x$}, and sometimes written $F_x$.
...
\end{tikzcd}\]
\begin{definition}
	Let $\pi : Y \to X$ be a locally trivial fibration. Then:
	\begin{enumerate}
		\item If all $\pi^{-1}[x]$ are homeomorphic, we call $\pi$ a \tib{fiber bundle}.
		\item If every $\pi^{-1}[x]$ is discrete, we call $\pi$ a \tib{covering map}.
		\item If we can pick $U = X$, $\pi$ is called a \tib{trivial fibration}.
	\end{enumerate}
\end{definition}
\begin{exercise}
...
\end{exercise}
\begin{lemma}
	Let $\pi : Y \to X$ be a locally trivial fibration. Let $X$ be connected. Then $\pi$ is a trivial fibration.
\end{lemma}
\begin{proof}
	Let $x_0 \in X$. Let $V = \lr{x \in X \mid F_x \cong F_{x_0}}$ be the set of all points whose fibers are homeomorphic to the fiber of $x_0$. Let $U$ be a neighborhood as in the definition. Then the fibers of all $x \in U$ must be homeomorphic by definition. Thus, every point in $V$ has a neighborhood contained in $V$, so $V$ is open. We have
	\begin{align*}
		X \setminus V = \bigcup_{y \notin V} \lr{x \in X \mid F_x \cong F_y}
		:=
		 \bigcup_{y \notin V} V_y
	\end{align*}
	Since every $V_y$ is open by the same argument, $X \setminus V$ must be open. But since $X$ is connected with $(X \setminus V) \cup V = X$ and $V$ nonempty, $X \setminus V$ must be empty.
\end{proof}
\begin{theorem}
	A continuous function $\pi : Y \to X$ is a covering map if and only if every $x \in X$ has an open neighborhood $U$ such that $\pi^{-1}[U]$ can be decomposed into disjoint open sets $V_i \subseteq Y$ such that $\pi|_{V_i}$ is a homeomorphism $V_i \cong U$.
\end{theorem}
\begin{proof}
	\begin{enumerate}
		\item[$\Rightarrow$:] Let $\pi$ be a covering map. Let $I = F_U$. Since $h_U$ is bijective, the sets $V_i := h_U^{-1}[U \times \lr{i}]$ are disjoint. Since $h_U$ is a homeomorphism and $I = F_U$ is discrete, the $V_i$ are also open. Since $p_1|_{U \times \lr{i}}$ is a homeomorphism $U \times \lr{i} \cong U$, the composition
		\begin{align*}
			\pi|_{V_i} = (p_1|_{U \times \lr{i}}) \circ h|_{V_i}
		\end{align*}
		must be a homeomorphism $V_i \mapsto U$.
		\item[$\Leftarrow$:] Define 
		\begin{align*}
			h_U : \pi^{-1}[U] &\cong U \times I\\
			y \in V_i &\mapsto (\pi(y),i).
		\end{align*}
		Then the restriction of $h_U$ to each $V_i$ is by definition a homeomorphism $V_i \cong U \cong U \times \lr{i}$, and so the whole of $h_U$ is a homeomorphism $\pi^{-1}[U] = \bigsqcup_{i \in I} V_i \cong \bigsqcup_{i \in I} U \times \lr{i} = U \times I$.
	\end{enumerate}
\end{proof}
