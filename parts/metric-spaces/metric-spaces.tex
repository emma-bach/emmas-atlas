...
\begin{proof}
	\begin{enumerate}
		\item
		\item Let $K \subseteq M$ be compact. Since $M$ is a metric space, $K$ is closed. If we cover $K$ using open balls, that cover must have a finite subcover. Thus, $K$ must be contained in a finite union of open balls, which means $K$ must be bounded.
	\end{enumerate}
\end{proof}
\begin{theorem}
...
\end{importanttheorem}
\begin{proof}
	A bounded set is a subset of a cube $C^n = \prod_{i = 1}^n [-r,r]$. Such a cube is a product of closed intervals, which are compact by Theorem \ref{th:compactinterval}. Every set that is closed and bounded is thus a closed subset of a compact set, making them compact by Exercise \ref{th:subsetcompact}.
\end{proof}
\begin{theorem}
...
\begin{exercise}
	\begin{enumerate}
		\item Show that $\bN$ is not connected.
		\item Show that $\bQ$ is not connected.
	\end{enumerate}	
\end{exercise}
\begin{exercise}
