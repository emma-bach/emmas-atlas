...
\end{proof}
\begin{theorem}
	\label{th:regularclosedneighbourhoods}
	A Hausdorff space is regular if and only if the closed neighbourhoods of any point form a neighbourhood basis of that point.
\end{theorem}
...
\end{exercise}
\begin{exercise}
	\label{th:compacthausdorffnormal}
...
\begin{importantcorollary}{Urysohn's Metrization Theorem}{urysohnmetrization}
	Every second-countable completely regular Hausdorff space $X$ can be embedded into the Hilbert Cube
	\begin{align*}
		\prod_{n \in \bN} [0,1]
	\end{align*}
...
\section{Local Compactness and Compactification}
\begin{importantdefinition}{}{}
	We call a topological space \tib{locally compact} if every point has a compact neighbourhood.
\end{importantdefinition}
\begin{theorem}
	Let $X$ be a locally compact Hausdorff space. Then:
	\begin{enumerate}
		\item $X$ is completely regular.
		\item Every neighbourhood of $x \in X$ contains a compact neighbourhood of $x$.
	\end{enumerate} 
\end{theorem}
\begin{proof}
	Let $C$ be a compact neighbourhood of $x$. Let $U$ be an arbitrary neighbourhood of $x$. Let $V \subseteq C \cap U$ be open with $x \in V$. Then $\ol V \subseteq C$ is a compact Hausdorff space, and thus normal by Exercise \ref{th:compacthausdorffnormal} and completely regular by Urysohn's Lemma \ref{th:urysohnlemma}. Since every point has a completely regular neighbourhood, $X$ is completely regular.
	\newpar
	Since $V \subseteq \ol V$ is also completely regular, the closed neighbourhoods of any point $x \in V$ form a neighbourhood basis by Theorem \ref{th:regularclosedneighbourhoods}. 
	\newpar
	Thus, there exists a neighbourhood $N \subseteq V$ of $x \in C$ which is closed in $\ol V$, and thus closed in $X$. Since $N \subseteq C$ is closed and $C$ is compact, $N$ is also compact. Since $N$ is a neighbourhood of $x$ in $\ol V$ and $N \subseteq V$,< $N$ is also a neighbourhood of $x$ in the open set $V$, and thus in all of $X$.
\end{proof}
\begin{importanttheorem}{One-Point Compactification}{onepointcompactification}
	Let $(X, \cO)$ be a locally compact Hausdorff space. Let $X^+ = X \cup \lr{\infty}$ be the set consisting of $X$ and an additional "point at infinity" not originally contained in $X$. 
	\newpar
	Define the collection of open sets in $X^+$ to be
	\begin{align*}
		\cO^+ := \cO \cup \lr{X^+ \setminus C \mid C \subseteq X \text{ compact}},
	\end{align*}
	i.e. the original open sets in $X$ plus the complements of compact sets in $X$ with the additional point $\infty$ added. Then $(X^+, \cO^+)$ is a compact Hausdorff space called the \tib{one point compactification} of $X$. Furthermore, $\cO^+$ is the only topology making the set $X^+$ a compact Hausdorff space with $X$ as a subspace.
\end{importanttheorem}
\clearpage
...
\end{proof}
\begin{corollary}\label{th:paracompacthausdorffnormal}
...
\end{proof}
Thus, every second countable paracompact space is metrizable by Urysohn's Lemma \ref{th:urysohnlemma}.
\begin{proposition}
...
\end{proofsketch}
\begin{importantdefinition}{}{}
	Let $f : X \to \bR$ be a continuous map from a topological space into the real numbers. Then the \tib{support of $f$} is the smallest closed set on which $f(x) \neq 0$:
	\begin{align*}
		\supp f = \ol{\lr{x \in X \mid f(x) \neq 0}}
	\end{align*}
\end{importantdefinition}
\begin{definition}
	Let $\cU = \lr{U_i \mid i \in I}$ be an open covering of the space $X$. Then a \tib{partition of unity} subordinate to this covering is a collection of maps
	\begin{align*}
		\lr{f_j : X \to [0,1] \mid j \in J}
	\end{align*}
	such that:
	\begin{enumerate}
		\item There exists a locally finite open refinement $\lr{V_j \mid j \in J}$ of $\cU$ such that
		\begin{align*}
			\forall j \in J : \supp f_j \subseteq V_j
		\end{align*}
		\item The functions "partition unity" in the sense that they add up to the constant function $\boldone_X$:
		\begin{align*}
			\forall x \in X : \sum_{j \in J} f_j(x) = 1
		\end{align*}
	\end{enumerate}
\end{definition} 
\begin{theorem}
	Let $X$ be a paracompact topological space. Let $\cU$ be an open covering of $X$. Then there exists a partition of unity subordinate to $\cU$.
\end{theorem}
\begin{corollary}
	Let $X$ be paracompact. Let $\lr{U_i \mid i \in I}$ be a locally finite open covering of $X$. Then there exists an open covering $\lr{V_i \mid i \in I}$ such that for each $i \in I$, we have $\ol V_i \subseteq U_i.$
\end{corollary}
\begin{definition}
	We call a topological space \tib{$\sigma$-compact} if it is the union of countably many compact subspaces.
\end{definition}
\begin{theorem}{\label{th:sigmapara}}
	A locally compact Hausdorff space is paracompact if and only if it is the disjoint union of open $\sigma$-compact subsets.
\end{theorem}
\begin{theorem}
	Let $X$ be a locally compact second countable Hausdorff space. Then $X$ is $\sigma$-compact and paracompact.
\end{theorem}
In particular, $X$ is a second countable completely regular Hausdorff space by Corollary \ref{th:paracompacthausdorffnormal}, and thus metrizable by Urysohn's metrization theorem \ref{th:urysohnmetrization}.
\begin{proof}
	Let $X$ be a locally compact second countable Hausdorff space.
	Let $\cB$ be a countable basis of $X$. Let $C \subseteq X$ be compact. By local compactness, every point $x \in C$ has a compact neighbourhood $N$. Since $N$ is a neighbourhood, there exists a basis element $U_x \in \cB$ such that $x \in U_x \subseteq N$. Since $U_x \subseteq N$, each closure $\ol U_x$ is compact, so the closures of these basis elements give a countable covering of $X$ by compact sets, i.e. $X$ is $\sigma$-compact. By Theorem \ref{th:sigmapara}, $X$ is also paracompact.
\end{proof}
\begin{theorem}
	If $X$ is a locally compact second countable Hausdorff space, then its one-point compactification $X^+$ is metrizable.
\end{theorem}
\begin{proof}
	Continuing the construction from the previous proof, we can cover $C$ by the our basis elements $U_x$ and pick a finite subcover $\lr{U_{x_k} \mid k \in [1..n]}$.
	\newpar
	Then $\bigcup_{k = 1}^n \ol{U_{x_k}}$ is a finite union of compact sets, i.e. compact, so the set 
	\begin{align*}
		V_C = X^+ \setminus \bigcup_{k = 1}^n \ol{U_{x_k}}
	\end{align*} 
	is an open neighbourhood of $\infty$ contained in an arbitrary neighbourhood of the form $X^+ \setminus C$. 
	\newpar
	Each set $V_C$ is uniquely defined by a finite subset of the countable set $\cB$, so there are only countably many $V_C$. Thus, $\cB \cup \lr{V_C \mid C \text{ compact}}$ is a countable basis of $X^+$. Furthermore, $X^+$ is a compact Hausdorff space by construction, and thus completely regular. Metrizability follows, as usual, from Urysohn's metrization theorem \ref{th:urysohnmetrization}.
\end{proof}
\clearpage
...
\begin{enumerate}
	\item $F_X(U) = C(U, \bR^n)$,
	\item $F_X(U) = C^k(U, \bR^n)$,
	\item $F_X(U) = C^\infty(U, \bR^n)$,
	\item $F_X(U) = A(U, \bR^n)$ ($=$ the set of analytic functions $U \to \bR^n$).
\end{enumerate}
\begin{exercise}
...
\begin{proof}
	\begin{enumerate}
		...
		\begin{align*}
			F(U) = \lr{f \circ \Phi \mid f \in C^\infty(U', \bR^n)}
		\end{align*}
		Then $F$ inherits the properties defining a functional structure from $C^\infty(U', \bR^n)$, and $\Phi$ is an isomorphism between $F$ and $C^\infty$ by definition of $F$.
...
\begin{proof}
	\begin{enumerate}
		\item[$\Rightarrow$:] Let $f : (M, F_M) \to (N, F_N)$ be a morphism. Then we have:
		\begin{align*}
			\Psi \in C^\infty(V, V') 
			&\implies \Psi \circ f \in F_M(f^{-1}(V))\\
			&\implies \Psi \circ f \circ \Phi^{-1} \in C^\infty(\Phi(f^{-1}(V)), \bR^n)
		\end{align*}
		\item[$\Leftarrow$:] Let $\Psi \circ f \circ \Phi^{-1} \in C^\infty(U', V')$. In order for this function to be defined, we need 
		\begin{align*}
			f(\Phi^{-1}(U')) = f(U) \subseteq V = \Psi^{-1}(V')
		\end{align*}
		Since the restriction of a chart is itself a valid chart, $\phi := \Phi|_{f^{-1}(V)}$ is a chart $f^{-1}(V) \to \phi(f^{-1}(V))$, and thus an isomorphism.
		Thus, we have
		\begin{align*}
			& \Psi \in C^\infty(V, V')\\
			\implies &\Psi \circ f \circ \phi^{-1} \in C^\infty(\phi(f^{-1}(V)), V')\\ 
			\implies &\Psi \circ f = \Psi \circ f \circ \phi^{-1} \circ \phi \in F_M(\phi^{-1}(\phi(f^{-1}(V)))) = F_M(f^{-1}(V)),
		\end{align*}
		so $f$ is a morphism.
	\end{enumerate}
\end{proof}
