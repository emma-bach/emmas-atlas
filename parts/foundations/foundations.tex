...
\newpar
These ground rules are given in the form of \tib{axioms}, which are statements that mathematicians take for granted, which can then be used as a starting point to then derive "the rest of mathematics" from. The following chapters contain a brief and hopefully easily understandable introduction to the ZFC axiom system, which was developed in the early 20th century by Ernst Zermelo and Abraham Fraenkel and is now the most widely accepted axiomatization of mathematics. Informally stated, the ZFC axioms are:
\begin{enumerate}
	\item The empty set is a set, and the natural numbers are a set.
	\item Sets are uniquely determined by their elements.
	\item Any definable subset of a set is a set.
	\item Given two sets, there exists a set whose elements are those two sets.
	\item Given two sets, there exists a set containing the elements of both sets.
	\item Given a set, there exists a set containing all subsets of that set as elements.
	\item The image of a set under a definable function is a set.
	\item There are no infinitely nested sets.
	\item Given any set of nonempty sets, there exists a function mapping each sets to one of its elements.
\end{enumerate}
\section{Infinity: $\emptyset$ and $\bN$}
\section{Extensionality}
To say "$x$ is an element of $M$", or equivalently "$x$ contains $M$", we simply write:
\begin{align*}
	x \in M
\end{align*}
...
\begin{align*}
	A = B \Leftrightarrow A \subseteq B \tn{ and } B \subseteq A
\end{align*}
...
We can generalize the cartesian product to infinite products as follows:
